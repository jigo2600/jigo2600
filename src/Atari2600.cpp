...
bool Atari2600State::operator==(Atari2600State const& s) const {
  return cmp(cpu, s.cpu) && cmp(pia, s.pia) && cmp(tia, s.tia) &&
...
/// Run the simulation until `maxNumCPUClocks` have been executed, a new frame
/// is generated, or a breakpoint is reached, depending which events occur
/// first. Note that more than one of these criteria can be met at the same
/// time; the function returns all reasons why it stopped.

Atari2600::StoppingReason Atari2600::cycle(size_t& maxNumCPUCycles) {
  // Handle inputs.
  syncPorts();
  ...
  while (!reason.any()) {
    ...
    if (_cpu->getT() == 0) {
      ...
      //
      // The breakpoint list is scanned to check for a hit after the *next*
      // cycle is executed.
...
shared_ptr<Atari2600State> Atari2600::makeState() const {
...
shared_ptr<Atari2600State> Atari2600::saveState() const {
  auto s = make_shared<Atari2600State>();
...
void Atari2600::setVideoStandard(VideoStandard type) {
...
Atari2600::VideoStandard Atari2600::getVideoStandard() const {
  return getTia()->getVideoStandard();
}

/// Set the state of the console panel switches.
void Atari2600::setPanel(Panel panel) {
  this->panel = panel;
}

/// Get the state of the console panel switches.
Atari2600::Panel Atari2600::getPanel() const {
  return panel;
}

void Atari2600::setJoystick(int num, Joystick joystick) {
  assert(num == 0 || num == 1);
...
void Atari2600::setPaddle(int num, Paddle paddle) {
  assert(0 <= num && num < 4);
  paddles[num] = paddle;
  inputType = InputType::paddle;
}

void Atari2600::setKeyboard(int num, Atari2600::Keyboard keys) {
  assert(0 <= num && num < 2);
  keyboards[num] = keys;
...
void Atari2600::syncPorts() {
  ...
  case InputType::joystick: {
    auto setj = [&](int num) {
      return (1 << 3) * !joysticks[num][Joystick::right] +
  ...
  case InputType::paddle: {
    ...
    pia.writePortA((1 << 7) * !paddles[0].fire + (1 << 6) * !paddles[1].fire +
                   (1 << 3) * !paddles[2].fire + (1 << 2) * !paddles[3].fire);
    break;
  }
...
uint32_t Atari2600::virtualizeAddress(uint16_t address) const {
...
uint8_t* Atari2600::mutableDataForVirtualAddress(std::uint32_t address) {
  uint8_t const* data = dataForVirtualAddress(address);
  return (uint8_t*)(data);
}

map<uint32_t, Atari2600BreakPoint> const& Atari2600::getBreakPoints() {
  return breakPoints;
}
...
std::ostream& Atari2600::printInstruction(std::ostream& os,
                                          M6502::Instruction const& ins) {
