...
struct Atari2600State {
  // Lifecycle.
  Atari2600State();
  Atari2600State(std::shared_ptr<M6502State> cpu, std::shared_ptr<M6532State> pia,
...
public:
  ...
  Atari2600Cartridge* getCartridge() const {
    return dynamic_cast<Atari2600Cartridge*>(cartridge.get());
  }
  ...
  uint8_t* mutableDataForVirtualAddress(
      std::uint32_t virtualAddress); // todo: think if we really want this
  ...
  struct Paddle {
    Paddle() = default;
    Paddle(bool fire, float angle) : fire(fire), angle(angle) {}
    bool fire;
    float angle;
  };
