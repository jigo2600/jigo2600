...
ostream& printHelper(ostream & os, M6502::InstructionTraits const& ins, T operand)
{
  auto w = os.width() ;
  auto f = os.flags() ;
  auto l = os.fill() ;
...
void M6502::reset()
{
  X = 0xc0 ; // as visual6502?
  S = 0x00 ;
  P[P.i] = true ; // IRQs are disabled on reset
  P[P.z] = true ; // as visual6502?
  IR = 0x00 ; // as visual6502?
  TP = 1 ;
  dc = decode(IR) ;
  numCycles = 0 ;
...
uint8_t M6502::xADC(uint8_t operand)
{
  ...
  if (P[P.d] == false) {
    ...
    if (nibble0sum >= 0xa) {
      sum += 0x06 ;
      if (nibble0sum >= 0x1a) { sum -= 0x10 ; } // For non-valid BCD.
    }
    P[P.n] = (sum & 0x80) ;
    P[P.v] = ~(A ^ operand) & (A ^ sum) & 0x80 ;
    if ((sum & 0x1f0) >= 0xa0) { sum += 0x60 ; }
    P[P.c] = (sum >= 0x100) ;
  }
...
uint8_t M6502::xSBC(uint8_t operand)
{
  ...
  if (P[P.d] == false) {
    ...
    if (nibble0diff >= 0x10) {
      diff -= 0x6 ;
      if (nibble0diff <= 0x15) { diff += 0x10 ; } // For non-valid BCD.
    }
    P[P.n] = (diff & 0x80) ;
    P[P.v] = (A ^ operand) & (A ^ diff) & 0x80 ;
    if ((diff & 0x1f0) >= 0x100) { diff -= 0x60 ; }
    P[P.c] = (diff >= 0) ; // Complement of carry.
  }
...
#define fetch() {readFrom(PC); PCP = PC + 1;}
...
void M6502::cycle(bool busWasReady)
{
  ...
  if (!busWasReady && RW) {
    ...
    // the read.
    ++ numCycles ;
    if (verbose) { cout << "M6502: Repeating cycle because of stall." << endl ; }
    return ;
  }
  ...
  if (T == 1) {
    // Some instruction executiong ``spill'' to T = 1 despite the fact
    // that the opcode changes during this period.
    // This is used to move the internal ADD to another registes
    // as needed.
    if (dc.addToA) { A = ADD ; }
    else if (dc.instructionType == DEX) { X = ADD ; }
    else if (dc.instructionType == INX) { X = ADD ; }
    else if (dc.instructionType == DEY) { Y = ADD ; }
  ...
  if (dc.accessType == noAccess) {
    if (T == 1) {
      ...
      if (dc.instructionType == KIL) {PCP = PC + 1;}
      else {TP = -1 ;}
    }
    else if (T == 2) {readFrom(0xfffe); TP = 1;} // KIL.
    else if (T == 0) {
  ...
  else if (dc.accessType == read ||
           dc.accessType == write ||
           dc.accessType == readWrite) {
    int Tx ;
    switch (dc.addressingMode) {
        // On T==Tx this code is completed below using AD.
      case immediate:
        Tx = 1 ;
        if (T == 1) {AD = PC; PCP = PC + 1;}
        break ;

      case zeroPage:
        Tx = 2 ;
        if (T == 1) {fetch();}
        else if (T == 2) {AD = dataBus;}
        break ;
      ...
      case zeroPageIndexedIndirect: // (opcode,X)
        ...
        else if (T == 3) {readFrom(ADD);}
        else if (T == 4) {readFrom(uint8_t(ADD + 1)); AD = dataBus;}
        else if (T == 5) {AD |= uint16_t(dataBus) << 8;}
        break ;

      case zeroPageIndirectIndexed:
        Tx = 5 ;
        if (T == 1) {fetch();}
        else if (T == 2) {readFrom(AD = dataBus);}
        else if (T == 3) {
          ...
          if (dc.accessType == read && !carry) {T=++TP;} // Skip step.
          else {
            readFrom(AD) ; // Discarded.
      ...
      case absolute:
        Tx = 3 ;
        if (T == 1) {fetch();}
        else if (T == 2) {AD = dataBus; fetch();}
        else if (T == 3) {AD |= uint16_t(dataBus) << 8;}
        break ;
        ...
        if (T == 1) {fetch();}
          ...
          if (dc.accessType == read && !carry) {T=++TP;} // Skip step.
            ...
            if (carry) {
              AD += 0x100;
              if (dc.instructionType == AHX ||
                  dc.instructionType == SHX ||
                  dc.instructionType == SHY ||
    ...
    if (dc.accessType == read) {
      ...
      if (T == Tx) {
        switch (dc.instructionType) {
          ...
          case TAS: case SHX: case SHY: case AHX: writeTo(AD, ADD) ; break ;
          default: assert(false) ;
        }
        TP = -1 ;
      }
        ...
        // Illegal opcode.
        if (dc.instructionType == TAS) {S = A & X;}
      }
    }
      ...
      if (T == Tx) {readFrom(AD);}
        ...
        switch (dc.instructionType) {
          case ASL: case SLO: ADD = xASL(dataBus) ; break ;
          case DEC: case DCP: ADD = setNZ(dataBus - 1) ; break ;
          case INC: case ISC: ADD = setNZ(dataBus + 1) ; break ;
          case LSR: case SRE: ADD = xLSR(dataBus) ; break ;
          case ROL: case RLA: ADD = xROL(dataBus) ; break ;
          case ROR: case RRA: ADD = xROR(dataBus) ; break ;
          default: assert(false) ;
        }
        writeTo(AD,dataBus); // Discarded.
      }
      else if (T == Tx+2) {writeTo(AD,ADD); TP = -1;}
      else if (T == 0) {fetch();
        // Illegal opcodes.
        switch (dc.instructionType) {
    ...
    if (dc.addressingMode == absolute) {
      ...
      else if (T == 3) {AD |= uint16_t(dataBus) << 8; readFrom(AD);}
      else if (T == 4) {
    ...
    if (T == 1) {fetch();}
    else if (T == 2) {AD = dataBus; readFrom(0x100 + S) ;}
    else if (T == 3) {writeTo(0x100 + S--, (PC >> 8) & 0xff) ;}
    else if (T == 4) {writeTo(0x100 + S--, PC & 0xff) ;}
    else if (T == 5) {fetch(); TP = -1;}
    else if (T == 0) {
      PC = (AD |= uint16_t(dataBus) << 8) ;
      fetch() ;
    }
    ...
    else if (T == 3) {readFrom(0x100 + ++S); TP = -1;}
    else if (T == 0) {
    ...
    if (resetLine) { low = 0xfffc ; high = 0xfffd ; b = false ; }
    else if (irqLine) { low = 0xfffe ; high = 0xffff ; b = false ; }
    else if (nmiLine) { low = 0xfffa ; high = 0xfffb ; b = false ; }
    else { low = 0xfffe ; high = 0xffff ; }
    //
    if (T == 1) {fetch();} // Discarded.
    if (T == 2 && !resetLine) {writeTo(0x100 + S, (PC>>8) & 0xff);}
    else if (T == 2) {readFrom(0x100 + S);}
    else if (T == 3 && !resetLine) {writeTo(0x100 + uint8_t(S-1), PC & 0xff);}
    else if (T == 3) {readFrom(0x100 + uint8_t(S-1));}
    else if (T == 4 && !resetLine) {writeTo(0x100 + uint8_t(S-2), getP(b));}
    else if (T == 4) {readFrom(0x100 + uint8_t(S-2));}
    else if (T == 5) {S -= 3; readFrom(low);  P[P.i] = true;}
    else if (T == 6) {
      AD = dataBus ; readFrom(high) ;
    ...
    else if (T == 4) {AD = dataBus; readFrom(0x100 + S);}
    else if (T == 5) {
      PC = (AD |= uint16_t(dataBus) << 8) ;
      fetch() ; // Discarded.
      TP = -1 ;
  ...
  if (verbose) {
    ...
    << setfill('0') << setw(4) << hex << currentPC << " "
    << setfill('0') << setw(2) << hex << (int)IR << "/"
    << setfill('0') << setw(2) << dec << currentT << " "
    << setfill('0') << setw(2) << hex << (int)currentDataBus << ","
    << setfill('0') << setw(2) << hex << (int)dataBus << (RW ? " R" : " W")
    << setfill('0') << setw(4) << hex << (int)addressBus << "  "
    << setfill(' ') << left << setw(15) << dc << " "
    << setfill('0') << right
    << "[A:" << setw(2) << hex << (int)A
    << " X:" << setw(2) << hex << (int)X
    << " Y:" << setw(2) << hex << (int)Y
