...
void from_json(const nlohmann::json& j, Atari2600Cartridge::Type& p) {
  if (j.is_null()) {
    ...
    if (str == "standard") {
      p = Atari2600Cartridge::Type::standard;
    } else if (str == "E0") {
      p = Atari2600Cartridge::Type::E0;
    } else if (str == "F0") {
      p = Atari2600Cartridge::Type::F0;
    } else if (str == "FE") {
      p = Atari2600Cartridge::Type::FE;
    } else {
...
class StateHelper : public virtual Atari2600CartridgeState {
public:
  Type getType() const override { return type; }

  bool operator==(StateHelper<State, type, ramSize> const& s) const { return true; }
...
class CartridgeHelper : public virtual Atari2600Cartridge, public State {
public:
  void reset() override { this->State::reset(); }

  void setVerbosity(int verbosity) override { this->verbosity = verbosity; }

  int getVerbosity() const override { return verbosity; }

  uint32_t getSize() const override { return romSize; }
  ...
  ConcreteAddress decodeVirtualAddress(uint32_t address) const override {
    ...
    ca.valid = (address & 0x1000) && (ca.regionNumber < getNumBanks()) &&
               (ca.regionOffset < romSize);
    return ca;
  }

  int getNumBanks() const override { return 1; }

  int getNumRegions() const override { return 1; }
...
class StandardState : public StateHelper<State, type>, public traits<type> {
public:
  using traits<type>::ramSize;

  StandardState() : activeBank(0) {}
...
class Standard : public CartridgeHelper<Cartridge, State, State::romSize> {
private:
...
public:
  ...
  uint32_t cycle(Atari2600& machine, bool chipSelect) override {
    ...
    uint32_t naddress =
        machine.getCpu()->getAddressBus() & ((romSize == 2_KiB) ? 0x07ff : 0x0fff);
  ...
  int getNumBanks() const override { return numBanks; }

  int getNumRegions() const override { return numBanks + (ramSize > 0); }
};

// Instantiate standard cartridge classes.
#define make(x)                                                               \
  class Atari2600Cartridge##x##State                                          \
   : public StandardState<Atari2600Cartridge##x##State, Type::x> {            \
  public:                                                                     \
    Atari2600Cartridge##x##State&                                             \
    operator=(Atari2600Cartridge##x##State const&) = default;                 \
  };                                                                          \
  class Atari2600Cartridge##x                                                 \
   : public Standard<Atari2600Cartridge##x, Atari2600Cartridge##x##State> {   \
  public:                                                                     \
...
public:
  Atari2600CartridgeE0State() : activeBanks{0, 0, 0} {}
...
class Atari2600CartridgeE0
 : public CartridgeHelper<Atari2600CartridgeE0, Atari2600CartridgeE0State,
                          Atari2600CartridgeE0State::romSize> {
public:
  void reset() override {
    this->super::reset();
  ...
  int getNumBanks() const override { return this->numBanks; }
...
private:
  using super = CartridgeHelper<Atari2600CartridgeE0, Atari2600CartridgeE0State,
                                Atari2600CartridgeE0State::romSize>;
};
...
public:
  Atari2600CartridgeFEState() : activeBank{0}, feDetected{false} {}

  bool operator==(Atari2600CartridgeFEState const& s) const {
    return super::operator==(s) && (activeBank == s.activeBank) &&
           (feDetected == s.feDetected);
  }
...
class Atari2600CartridgeFE
 : public CartridgeHelper<Atari2600CartridgeFE, Atari2600CartridgeFEState,
                          Atari2600CartridgeE0State::romSize> {
public:
  uint32_t cycle(Atari2600& machine, bool chipSelect) override {
  ...
  int getNumBanks() const override { return 2; }
...
template <typename T> bool is_member(const vector<T>& v, const T& x) {
...
jigo::makeCartridgeFromBytes(const char* begin, const char* end,
  ...
  if (type == Type::unknown || type == Type::standard) {
    ...
    for (uint8_t pattern : {0x00, 0xff}) {
      auto fillerSize =
          find_if(begin, min(end, begin + 512),
                  bind(not_equal_to<std::uint8_t>(), pattern, placeholders::_1)) -
          begin;
      if (fillerSize >= 512) {
        ramSize = 256;
...
#define m(x) \
  case Type::x: cart = mk<Atari2600Cartridge##x>(begin, end); break;
