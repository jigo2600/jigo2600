...
vector<M6502ByteType> jigo::tagM6502Memory(std::uint8_t const* begin,
    ...
    if (current == end) {
      ...
      for (int q = 0; q < 5; ++q) {
        if (q == Instruction0) {
          ...
          state.probability[q] =
              (i0.instructionType == M6502::UNKNOWN || i0.instructionType == M6502::KIL)
                  ? -inf
                  : 0;
        } else if (q == End) {
      ...
      for (int qp = 0; qp < 5; ++qp) {
        ...
        if (qp == End) {
          ...
          if (q == End) {
            ...
            p = log2(0.99f);
          } else if (q == Instruction0) {
            p = log2(0.0099f / numValidOpcodes);
          }
        } else {
  ...
  // Max out last element.
  {
    auto probs = std::array<float, 5>{};
    for (int q = 0; q < 5; ++q) {
      probs[q] = states.back().probability[q];
    }
    auto m = max_element(probs.begin(), probs.end());
...
M6502Disassembly jigo::disassembleM6502memory(std::uint8_t const* begin,
                                              std::uint8_t const* end) {
  auto tags = jigo::tagM6502Memory(begin, end);
  auto lines = M6502Disassembly();
  for (auto curr = begin; curr < end;) {
    array<uint8_t, 3> threeBytes{*curr, *min(curr + 1, end - 1), *min(curr + 2, end - 1)};

    M6502::Instruction ins = M6502::decode(threeBytes);
    auto line = pair<uint16_t, M6502::Instruction>(curr - begin, ins);
