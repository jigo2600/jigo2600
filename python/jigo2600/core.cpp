...
PYBIND11_MODULE(core, m) {
  ...
  m6502state
  .def_property("RW", &M6502State::getRW, &M6502State::getRW)
  ...
  .def_property("data_bus", static_cast<std::uint8_t (M6502State::*)() const>(&M6502State::getDataBus), &M6502State::setDataBus)
  ...
  .def_static("decode_bytes", static_cast<M6502::Instruction (*)(std::array<std::uint8_t,3> const&)>(&M6502::decode))
  ;
...
#undef it
  ...
  m6532state
    .def(py::init<>())
    .def_readwrite("ram", &M6532State::ram)
  ...
  .def("cycle", [](M6532& self, bool CS, bool RSnot, bool RW, std::uint16_t address, std::uint8_t data){
  ...
  tia
    .def(py::init<>())
    .def_readwrite("num_cycles", &TIA::numCycles)
  ;
  ...
  cartState
    .def_property_readonly("type", &Atari2600CartridgeState::getType)
  ;

  py::class_<Atari2600Cartridge, shared_ptr<Atari2600Cartridge> > cart (m, "Cartridge", cartState) ;
  cart
    .def_property_readonly("size", &Atari2600Cartridge::getSize)
